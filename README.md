...
- **Build host:** FreeBSD **amd64** (pfSense 2.8 / FreeBSD 15-CURRENT is fine) with:
  - **Go** (1.22+ recommended). On pfSense, Go is not installed by default. The **FreeBSD `lang/go` metaport is often missing** from pfSense package feeds; use the **versioned** package instead (name changes over time):
    ...
    **Toolchain / SIGSEGV:** upstream `amneziawg-go` may declare **`go 1.24.x`** in `go.mod`. Older `go123` (1.23.x) then tries to **download** `go1.24.x`, which can crash on some pfSense/15-CURRENT kernels. The helper **`scripts/build-amneziawg-go.sh`** patches **`go.mod`** to the host version (and drops **`toolchain`** lines), sets **`GOWORK=off`**, **`GOMAXPROCS=1`**, **`GODEBUG=asyncpreemptoff=1`**, and **`go build -p 1`**. For builds **without** vendored deps it uses **`GOTOOLCHAIN=local`** so the driver does not auto-upgrade the compiler (avoids that download path). When **`third_party/amneziawg-go/vendor`** is present, it uses **`GOTOOLCHAIN=auto`** so a **newer toolchain** can be installed while modules still come only from **`vendor/`** (needed if **`vendor/modules.txt`** requires Go **≥ 1.24** and the host is **1.23.x**). Override with **`AMNEZIAWG_GOTOOLCHAIN=local`** if even the toolchain download crashes. To skip patching: `make AMNEZIAWG_PATCH_GOMOD=0 clean pkg`.
